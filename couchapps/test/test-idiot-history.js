...
describe(ddName, () => {
  ...
  it('multiple things', async () => {
    ...
    const response = await db.query(ddName + '/' + viewName).catch(throwMessage);
    expect(response.total_rows).to.equal(2);
    expect(response.rows[0].key).to.deep.equal([thing1, timestamp, 'a']);
    expect(response.rows[0].value).to.equal(1);
    expect(response.rows[1].key).to.deep.equal([thing2, timestamp, 'a']);
    expect(response.rows[1].value).to.equal(2);
  });
